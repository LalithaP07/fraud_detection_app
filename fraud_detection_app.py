...
class FraudEmbedder:
    ...
    def create_transaction_narrative(self, row: pd.Series) -> str:
       """Create human-readable transaction narrative for BERT"""
        try:
            amount = round(row.get('amount', 0), 2)
            merchant = row.get('merchant_category', 'unknown')
            payment = row.get('payment_method', 'a payment method')
            location = row.get('location', 'an unknown location')
            time_of_day = row.get('time_of_day')
            day = row.get('day_of_week')

            # Optional readable day mapping
            day_map = {
                0: "Monday", 1: "Tuesday", 2: "Wednesday", 3: "Thursday",
                4: "Friday", 5: "Saturday", 6: "Sunday"
            }
            day_str = day_map.get(day, "an unknown day")

            # Format base sentence
            sentence = f"A transaction of ${amount} was made at an {merchant} merchant using {payment} in {location}"
    
            if time_of_day and time_of_day != 'unknown time':
                sentence += f" during the {time_of_day}"
            else:
                sentence += " at an unspecified time"
    
            sentence += f" on {day_str}"
    
            # Add velocity if available and meaningful
            velocity = row.get('velocity_1h', 0)
            if velocity and velocity > 0:
                velocity = int(velocity) if velocity == int(velocity) else round(velocity, 1)
                sentence += f", with {velocity} other transaction{'s' if velocity != 1 else ''} in the past hour"

            return sentence + "."
    
        except Exception as e:
            return "Transaction narrative could not be generated."
